...
while (grailList.length < totalGrail) {
  const number = Math.floor(Math.random() * totalCells) + 1;
  if (!grailList.includes(number)) grailList.push(number);
}
...
for (let i = 1; i <= totalCells; i++) {
  ...
  cell.addEventListener('click', function () {
    ...
    if (grailList.includes(i)) {
      // Se è un grail....
      cell.classList.add('cell-grail');
      otherEnd(true);
    }  
    else {
      // Se non lo è...
...
function otherEnd (isGrail){
  if(isGrail===true){
      endGameScreen.classList.add('grail');
